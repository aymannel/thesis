...
\includezxdiagram{chapter-3/commutation_trick}{0.62}

Recall that the Pauli $Y$ gate can be obtained by conjugating the Pauli $Z$ gate with Clifford gates (\ref{pauli-Y}). Using the $\pi$ copy (\ref{pi-copy}) and spider fusion (\ref{spider-fusion}) rules, we can show that the Pauli $Y$ can be expressed as a Pauli $X$ gate followed by a Pauli $Z$ gate (up to a global phase of $-i$).

\includezxdiagram{chapter-3/clifford_conjugation}{0.8}

We can derive how the same $\text{CNOT}$ gate interacts with the $\text{exp} \left[ - i\frac{\theta}{2} \left(Y \otimes X \right) \right]$ Pauli gadget by identifying how it gate interacts with the $Y \otimes X$ Pauli string.

\includezxdiagram{chapter-3/commutation_trick_YX}{0.95}

Similarly, identifying how single-qubit Clifford gates (\ref{clifford-definitions}) interact with Pauli gadgets amounts to identifying how they interact with the Pauli gates. Importantly, if the phase of the Pauli gate is flipped, we must also flip the phase of the corresponding Pauli gadget.

\includezxdiagram{chapter-3/single_qubit_clifford}{0.5}

PROPER EXAMPLE HERE BASED ON \ref{clifford-conjugation}

Using these rules, we are able to derive all of the commutation relations in the following figures. IMPORTANT FOR LATER DERIVATIONS etc
