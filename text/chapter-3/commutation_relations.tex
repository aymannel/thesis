...
Similarly, we can identify how the single-qubit Clifford gates (\ref{clifford-definitions}) interact with Pauli gadgets by identifying how they interact with the Pauli gates. In the example below, we use the fact that the Pauli $Y$ gate definitions above differ by a factor of $-1$ to show that commuting a Hadamard through a $Y$ leg flips the gadget's phase.
