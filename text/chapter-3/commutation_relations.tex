\section{Gadget Commutation Relations}%
...
Starting with the mismatching $Z$/$X$ pair and using the bialgebra (\ref{bialgebra}), Hadamard commutation (\ref{hadamard-commutation}) and spider fusion (\ref{spider-fusion}) rules, we can show that commuting the legs introduces a Hadamard between the Pauli gadget bodies \cite{Yeung2020}.
...
Since the Pauli $Y$ gate can be expressed as a Pauli $X$ gate followed by a Pauli $Z$ gate (up to a global phase of $-i$) we can identify the $\text{exp} \left[ - i\frac{\theta}{2} Y \otimes X \right] \text{CNOT}_{0, 1} = \text{CNOT}_{0, 1} \,\, \text{exp} \left[ - i\frac{\theta}{2} Y \otimes I \right]$ commutation relation.
...
\includezxdiagram{chapter-3/commutation_cz}{0.9}
