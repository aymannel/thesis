...
Each exponential terms can be implemented by a quantum circuit consisting of a CNOT ladder construction conjugated by a Clifford subcircuit. By sequentially composing these circuits (\ref{composition}), that is taking their matrix product, we obtain the following quantum circuit.
...
\begin{figure}[H]
    \centering
    \includezxdiagram{chapter-5/one_body4}{0.9}
...
Using the phase gadget result (\ref{phase-gadget-result}) to collapse the CNOT ladder constructions, we can show that the quantum circuit implementing a one-body excitation operator (Figure \ref{one-body-implementation}) corresponds to two commuting Pauli gadgets.
